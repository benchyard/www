...
VERSION_RE = re.compile(r"^(latest|[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?)$")
...
HERO_RELEASES = os.environ.get(
    "BENCHYARD_RELEASE_BASE", "https://hero.benchyard.com/releases"
).rstrip("/")
...
def release_bases(version: str) -> list[str]:
    folder = "latest" if version == "latest" else version
    github = (
        f"https://github.com/{REPOSITORY}/releases/latest/download"
        if version == "latest"
        else f"https://github.com/{REPOSITORY}/releases/download/{version}"
    )
    return [f"{HERO_RELEASES}/{folder}", github]
...
def _fetch_signed_release(version: str, work: Path) -> tuple[Path, Path]:
    manifest = work / "release-manifest.json"
    bundle = work / "release-manifest.sigstore.json"
    errors: list[str] = []
    for base in release_bases(version):
        try:
    ...
    raise RuntimeError("release manifest unavailable: " + "; ".join(errors))
...
def _image_present(ref: str) -> bool:
    return _run(["docker", "image", "inspect", ref], check=False).returncode == 0
...
def _compose_cmd() -> list[str]:
    ...
    if (COMPOSE_DIR / "compose.override.yaml").is_file():
        cmd.extend(["-f", str(COMPOSE_DIR / "compose.override.yaml")])
    return cmd


def _needed_images(parsed: dict[str, str]) -> list[str]:
    if ROLE == "worker":
        return [parsed["worker"]]
    return [parsed["api"], parsed["web"]]


def _ensure_stack(parsed: dict[str, str], services: list[str]) -> None:
    compose = _compose_cmd()
    pull = _run([*compose, "pull", *services], check=False)
    missing = [ref for ref in _needed_images(parsed) if not _image_present(ref)]
    if missing:
        detail = (pull.stderr or pull.stdout or "").strip()
        raise RuntimeError(
            f"images not on host: {', '.join(missing)}" + (f" ({detail})" if detail else "")
        )
    _run([*compose, "up", "-d", "--pull", "never", "--remove-orphans"])
...
def apply_release(version: str) -> str:
    ...
    manifest, bundle = _fetch_signed_release(version, work)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path == "/status":
            ...
            self._json(
                200,
                {"version": version, "busy": _state["busy"], "last_error": _state["last_error"]},
            )
            return
